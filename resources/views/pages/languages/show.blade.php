                ...
                <div class="card">
                    <div class="card-header align-items-center d-flex">
                        ...
                        <div class="flex-shrink-0">
                            <a type="button" class="btn btn-primary waves-effect waves-light" href="{{ route('working.schedule.create') }}"><i class="fa fa-plus-square-o"></i> დამატება</a>
                        </div>
                    </div><!-- end card header -->
                    <div class="card-body">
                            ...
                            <table id="working_schedule" class="table table-striped table-bordered" >
                                ...
                                @foreach($jsonArr as $key => $json)
                                    <tr>
                                        <td>{{ $key }}</td>
                                        <td>{{ $json }}</td>
                                        <td><input type="text" id="text-{{ $key }}"> <a href="javascript:void(0)" data-code="{{ $key }}" class="save-json">შენახვა</a></td>
                                    </tr>
                                @endforeach
                                </tbody>
                                <tfoot>
...
@push('js')
    ...
    <script>
        ...
        $(document.body).on('click', '.save-json', function () {
            let code = $(this).data('code');
            let text = $('#text-'+code).val();
            $.ajax({
                url: "{{ route('languages.update.json') }}",
                method: "POST",
                data: {
                    _token: '{{ csrf_token() }}',
                    'code': code,
