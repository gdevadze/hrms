                ...
                <div class="row">
                    <div class="col-12">
                        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                            <h4 class="mb-sm-0">დეპარტამენტები</h4>
            ...
            <div class="col-xl-12">
                <div class="card">
                    <div class="card-header align-items-center d-flex">
                        <h4 class="card-title mb-0 flex-grow-1">დეპარტამენტები</h4>
                        <div class="flex-shrink-0">
                            <a type="button" class="btn btn-primary waves-effect waves-light" id="add_department"
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            table = $('#departments').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('settings.departments.ajax') }}",
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                    },
        ...
        $(document.body).on('click', '#add_department', function () {
            ...
            $.ajax({
                url: "{{ route('settings.departments.create') }}",
                method: "POST",
                data: {
                    _token: '{{ csrf_token() }}',
                },
        ...
        $(document.body).on('click', '.edit_department', function () {
            $('#modal_form_detail').modal('show'); // show bootstrap modal when complete loaded
            $(".htmlDisplay").html('<h3 align=center class=text-warning><i class="fa fa-spinner fa-spin" style="font-size:24px"></i> @lang('wait')...</h3>');
            $.ajax({
                url: "{{ route('settings.departments.edit') }}",
                method: "POST",
                data: {
                    _token: '{{ csrf_token() }}',
                    'id': $(this).data('id')
